...
using FaceRecognition.Library.Models;
using FaceRecognition.Library.Utililties;
using System;
...
namespace FaceRecognition.Library.Algorithm
{
    class FaceRecognitionAlgorithm
    {
        ...
        private Image<Gray, byte> CropAndNormalizeFace(Image<Bgr, byte> inputImage, Face face)
        {
            ...
            {
                return thumbnailGrayFace.Resize(_faceSize.Width, _faceSize.Height, Inter.Cubic);
            }
        }

        private void LoadTrainingImages()
        {
            var trainingSet = _unitOfWork.Repository<FaceRecognition.Library.Models.Image>()
                .Query()
